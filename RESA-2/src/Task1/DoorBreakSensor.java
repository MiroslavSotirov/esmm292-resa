...
class DoorBreakSensor
{
	...
	public static void main(String args[])
	{
		...
		boolean DoorBreak;		    	// Current DoorBreak Situation
		...
		if (em != null)
		{
			...
			MessageWindow mw = new MessageWindow("DoorBreak Sensor", WinPosX, WinPosY);
			...
			mw.WriteMessage("\nInitializing DoorBreak Simulation::" );

			DoorBreak = false;


			mw.WriteMessage("   Initial DoorBreak Set:: " + DoorBreak );
			...
			*********************************************************************/
			...
			while ( !Done )
			{
				...
				PostDoorBreak(em, DoorBreak);

				mw.WriteMessage("Current DoorBreak Status:: " + DoorBreak + "%");
				...
				for ( int i = 0; i < qlen; i++ )
				{
					...
					{
						if (Msg.GetMessage().equalsIgnoreCase("D1")) // door break on
						{
							DoorBreak = true;

						} // if

						if (Msg.GetMessage().equalsIgnoreCase("D0")) // door break off
						{
							DoorBreak = false;
	...
	*
	*			 boolean doorBreak - this is the doorBreak value.
	*
	* Returns: none
	*
	* Exceptions: None
	...
	***************************************************************************/
	...
	static private void PostDoorBreak(MessageManagerInterface ei, boolean doorBreak)
	{
		// Here we create the message.

		Message msg = new Message( (int) 7, String.valueOf(doorBreak) );

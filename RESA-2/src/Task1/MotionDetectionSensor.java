...
class MotionDetectionSensor
{
	...
	public static void main(String args[])
	{
		...
		boolean MotionDetection;		    	// Current MotionDetection Situation
		...
		if (em != null)
		{
			...
			MessageWindow mw = new MessageWindow("MotionDetection Sensor", WinPosX, WinPosY);
			...
			mw.WriteMessage("\nInitializing MotionDetection Simulation::" );

			MotionDetection = false;


			mw.WriteMessage("   Initial MotionDetection Set:: " + MotionDetection );
			...
			*********************************************************************/
			...
			while ( !Done )
			{
				...
				PostMotionDetection(em, MotionDetection);

				mw.WriteMessage("Current MotionDetection Status:: " + MotionDetection + "%");
				...
				for ( int i = 0; i < qlen; i++ )
				{
					...
					{
						if (Msg.GetMessage().equalsIgnoreCase("M1")) // door break on
						{
							MotionDetection = true;

						} // if

						if (Msg.GetMessage().equalsIgnoreCase("M0")) // door break off
						{
							MotionDetection = false;
	...
	/***************************************************************************
	* CONCRETE METHOD:: PostMotionDetection
	...
	*
	*			 boolean motionDetect - this is the motionDetect value.
	*
	* Returns: none
	*
	* Exceptions: None
	...
	***************************************************************************/
	...
	static private void PostMotionDetection(MessageManagerInterface ei, boolean motionDetect)
	{
		// Here we create the message.

		Message msg = new Message( (int) 8, String.valueOf(motionDetect) );
